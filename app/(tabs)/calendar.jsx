...
import { getCurrentUser, getUserSchedules } from "../_lib/supabase";
...
const DARK_THEME = {
  ...
  medium: "#f59e0b",
  mediumTint: "rgba(245,158,11,0.15)",
  normal: "#22c55e",
...
const LIGHT_THEME = {
  ...
  medium: "#d97706",
  mediumTint: "rgba(217,119,6,0.12)",
  normal: "#16a34a",
...
function getCalendarSchedules(rankedSchedules) {
  return (Array.isArray(rankedSchedules) ? rankedSchedules : []).map((item) => {
...
export default function CalendarScreen() {
  ...
  useEffect(() => {
    ...
    const refreshPriorities = async () => {
      ...
      if (fingerprint === priorityFingerprintRef.current && fingerprint !== "") {
        return;
      }
      ...
      try {
        ...
        console.log("AI calendar priority ranking failed:", error?.message);
  ...
  const calendarSchedules = useMemo(
    () => getCalendarSchedules(prioritizedSchedules),
  ...
  const highPriorityThisMonth = calendarSchedules.filter((schedule) => {
    ...
    return (
      ...
      isHighPriority(schedule.priority)
    );
  }).length;
        ...
        <View style={styles.calendarCard}>
          ...
          <View style={styles.daysGrid}>
            {calendarCells.map((item) => {
              ...
              const hasHighPriority = daySchedules.some((schedule) =>
                isHighPriority(schedule.priority)
              );
              const hasMediumPriority = daySchedules.some((schedule) =>
                isMediumPriority(schedule.priority)
              );
        ...
        <View style={styles.selectedDateSection}>
          ...
          {loadingSchedules ? (
            ...
            selectedSchedules.map((item, index) => {
              ...
              const medium = isMediumPriority(item.priority);
              ...
              >
                ...
                <View style={styles.scheduleIconBox}>
                  <Feather
                    name={high ? "alert-circle" : medium ? "alert-triangle" : "book"}
                    size={22}
                    color={high || medium ? priorityColor : theme.primaryLight}
                  />
                </View>

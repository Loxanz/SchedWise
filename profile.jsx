...
export default function ProfileScreen() {
  ...
  const getProfileImageKey = (userId) => `schedwise_profile_image_${userId}`;
  ...
  const handleThemeChange = async (value) => {
    ...
    DeviceEventEmitter.emit(THEME_CHANGE_EVENT, nextTheme);
  ...
  const loadProfileImage = async (currentUser) => {
    ...
    try {
      const savedImage = await AsyncStorage.getItem(
        getProfileImageKey(currentUser.id)
      );

      setProfileImageUri(savedImage || null);
    } catch {
      setProfileImageUri(null);
    }
  };
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      loadThemeMode();
      loadTaskOverview();
    }, [loadThemeMode, loadTaskOverview])
  );
  ...
  const completedTaskItems = useMemo(() => {
    return schedules.filter((schedule) => Boolean(schedule?.completed));
  }, [schedules]);
  ...
  const taskListItems =
    taskListType === "completed" ? completedTaskItems : pendingTaskItems;

  const taskListTitle =
    taskListType === "completed" ? "Completed Tasks" : "Pending Tasks";
  ...
  const openTaskOverviewList = (type) => {
    setTaskListType(type === "completed" ? "completed" : "pending");
  ...
  const handlePickProfileImage = async () => {
    ...
    try {
      ...
      await AsyncStorage.setItem(getProfileImageKey(user.id), imageDataUri);
      setProfileImageUri(imageDataUri);

      Alert.alert("Profile Picture Updated", "Your new photo has been saved.");
    } catch (error) {
      Alert.alert(
        "Upload Failed",
      ...
      },
      {
        ...
        onPress: async () => {
          try {
            ...
            Alert.alert("Remove Failed", "Unable to remove profile picture.");
          }
        },
      },
    ]);
      ...
      >
        ...
        <View style={styles.content}>
          ...
          <View style={styles.overviewRow}>
            <TouchableOpacity
              style={[styles.overviewCard, styles.overviewCardLeft]}
              activeOpacity={0.82}
          ...
          <View style={[styles.modalCard, styles.taskListModalCard]}>
            ...
            {taskListItems.length > 0 ? (
              <ScrollView
                ...
                showsVerticalScrollIndicator={false}
              >
                {taskListItems.map((schedule) => {
                  const isCompleted = Boolean(schedule?.completed);

                  return (
                    <View key={schedule.id} style={styles.taskListItem}>
                      <View
                        style={[
              ...
              <View style={styles.taskListEmptyCard}>
                <View style={styles.taskListEmptyIcon}>
                  <Feather
                    name={
                      taskListType === "completed" ? "check-circle" : "clock"
                    }
                    size={24}
                    color={theme.primary}
                  />
                ...
                <Text style={styles.taskListEmptyTitle}>
                  No {taskListType === "completed" ? "completed" : "pending"}{" "}
                  tasks
                </Text>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <SettingSwitch
              ...
              subtitle="Notify me when deadlines are near"
            ...
            <SettingSwitch
              ...
              icon="sun"
              title="Daily Summary"
              subtitle="Show a daily academic reminder"
              value={dailySummary}
